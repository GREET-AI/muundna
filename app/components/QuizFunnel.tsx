...
import { trackPixelLead } from './LandingPixelsInjector';
import { ArrowLeft, ArrowRight, Check, Building2, Wrench, MapPin, Phone, Mail, Home, User } from 'lucide-react';
import IconInput from './ui/IconInput';
import Toast from './ui/Toast';

export interface QuizFunnelProps {
  /** Bei Produkt-Landing: Slug des Kurses/Produkts → eigener Funnel, Anfrage mit source product_quiz */
  productSlug?: string;
  /** Nach Erfolg: Link (z. B. zurück zur Produktseite) */
  successBackHref?: string;
  /** Text des Erfolgs-Links */
...
const BASE_STEPS = [
  { id: 'targetGroup', title: 'Ich bin...', subtitle: 'Mehrfachauswahl möglich', type: 'multiple', options: [
  ...
  { id: 'services', title: 'Ich interessiere mich für...', subtitle: 'Mehrfachauswahl möglich', type: 'multiple', options: [
  ...
  { id: 'timeframe', title: 'Wann benötigen Sie Unterstützung?', subtitle: 'Mehrfachauswahl möglich', type: 'multiple', options: [
  ...
  { id: 'location', title: 'Wo dürfen wir Sie beraten?', subtitle: '', type: 'single', options: [
...
export default function QuizFunnel({ productSlug, successBackHref = '/', successBackLabel = 'Zur Startseite' }: QuizFunnelProps) {
  const searchParams = useSearchParams();
  const paket = productSlug ? null : searchParams.get('paket'); // Paket nur auf Haupt-Quiz
  ...
  // Funnel-Tracking (nur bei Produkt-Quiz): Start + Formular-Ansicht
  useEffect(() => {
    if (!productSlug) return;
    trackFunnelEvent(productSlug, 'quiz_start');
  }, [productSlug]);
  useEffect(() => {
    if (productSlug && showForm) trackFunnelEvent(productSlug, 'quiz_form_view');
  }, [productSlug, showForm]);

  const handleOptionSelect = (stepId: string, optionId: string, type: string) => {
    setQuizData((prev) => {
      if (type === 'multiple') {
        const current = (prev[stepId as keyof QuizData] as string[]) || [];
        const newValue = current.includes(optionId) ? current.filter((id) => id !== optionId) : [...current, optionId];
  ...
  const handleNext = () => {
    if (productSlug) {
      ...
      trackFunnelEvent(productSlug, 'quiz_step_ok', { stepIndex: currentStep, stepId: step?.id });
    }
  ...
  const handleFormSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const quizPayload = {
        ...
        ...(productSlug && { source: 'product_quiz' as const, productSlug }),
      };
      const response = await fetch('/api/contact', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ ...formData, quizData: quizPayload }),
      });
      const data = await response.json();
      if (response.ok) {
        if (productSlug) {
          trackFunnelEvent(productSlug, 'quiz_complete');
          trackPixelLead(productSlug);
        }
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-[#fff3e7] via-white to-[#ffe1c7] relative overflow-hidden quiz-bg">
            ...
            {!showForm && !submitSuccess && (
              <div className="mb-8">
                <div className="flex justify-between mb-2">
                  <span className="text-sm text-gray-600">Schritt {currentStep + 1} von {steps.length}</span>
            ...
            <AnimatePresence mode="wait">
              {submitSuccess ? (
                ...
                >
                  ...
                  <h2 className="text-xl sm:text-2xl md:text-3xl font-bold text-gray-800 mb-3 sm:mb-4 break-words">Vielen Dank! 🎉</h2>
                ...
                >
                  <div className="mb-6 md:mb-8">
                    <h2 className="text-xl sm:text-2xl md:text-3xl font-bold text-gray-800 mb-2 break-words">Letzter Schritt 👋</h2>
                  ...
                  <form onSubmit={handleFormSubmit} className="space-y-6">
                    ...
                    <IconInput icon={<Mail className="w-5 h-5" />} name="email" type="email" placeholder="E-Mail Adresse" required value={formData.email} onChange={(e) => setFormData({ ...formData, email: e.target.value })} />
                    <IconInput icon={<Phone className="w-5 h-5" />} name="phone" type="tel" placeholder="Ihre Telefonnummer" value={formData.phone} onChange={(e) => setFormData({ ...formData, phone: e.target.value })} />
                    <IconInput icon={<Home className="w-5 h-5" />} name="street" type="text" placeholder="Straße & Haus-Nr." value={formData.street} onChange={(e) => setFormData({ ...formData, street: e.target.value })} />
                    <IconInput icon={<MapPin className="w-5 h-5" />} name="city" type="text" placeholder="Ihr Wohnort" value={formData.city} onChange={(e) => setFormData({ ...formData, city: e.target.value })} />
                    <div className="flex items-start">
                      ...
                      <label htmlFor="privacy" className="text-sm text-gray-700 cursor-pointer">
                        Datenschutzbestimmungen gelesen und akzeptiert * <a href="/datenschutz" className="text-[#182c30] hover:underline ml-1">(Datenschutzerklärung)</a>
                      </label>
                    </div>
                    <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3 sm:gap-4 pt-4">
                      <button type="button" onClick={handleBack} className="flex items-center justify-center px-4 sm:px-6 py-2.5 sm:py-3 text-sm sm:text-base text-gray-700 hover:text-[#cb530a] transition-colors">
                        <ArrowLeft className="w-4 h-4 sm:w-5 sm:h-5 mr-2" /> Zurück
                      </button>
                      <button type="submit" disabled={isSubmitting} className="flex-1 flex items-center justify-center px-4 sm:px-6 py-2.5 sm:py-3 bg-[#cb530a] text-white text-sm sm:text-base font-semibold rounded-lg hover:bg-[#a84308] transition-colors disabled:opacity-50 disabled:cursor-not-allowed whitespace-nowrap">
                        {isSubmitting ? 'Wird gesendet...' : (<>Jetzt anfragen<svg className="w-5 h-5 ml-2" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5l7 7-7 7" /></svg></>)}
                      </button>
                    </div>
                  </form>
                </motion.div>
              ) : (
                <motion.div key={currentStep} initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="grid grid-cols-1 lg:grid-cols-2 gap-12">
                  <div className="bg-white/95 backdrop-blur-sm rounded-2xl shadow-xl p-4 sm:p-6 md:p-8 border border-white/70">
                    <h2 className="text-lg sm:text-xl md:text-2xl font-bold text-[#cb530a] mb-3 break-words">Qualität aus Verantwortung.</h2>
                    <h3 className="text-base sm:text-lg font-semibold text-gray-800 mb-3 break-words">Warum für Muckenfuss & Nagel entscheiden?</h3>
                    <p className="text-gray-700 text-sm sm:text-base mb-4">
                      Wählen Sie Muckenfuss & Nagel für innovative Lösungen und erstklassigen Service. Unsere Bürodienstleistungen kombinieren Branchenkenntnis mit modernster Technologie.
                    </p>
                    <ul className="space-y-2 text-sm">
                      {['Made in Germany – Höchste Qualität.', 'Branchenkenntnis – 10+ Jahre Bau-Erfahrung.', 'Maßgeschneidert – Individuelle Lösungen.', 'Zuverlässig – Pünktliche Lieferung.', 'Transparent – Klare Dokumentation.', 'Eigener Ansprechpartner – Für jedes Projekt'].map((item, index) => (
                  ...
                  <div className="bg-white/95 backdrop-blur-sm rounded-2xl shadow-xl p-4 sm:p-6 md:p-8 border border-white/70">
                    <h2 className="text-lg sm:text-xl md:text-2xl font-bold text-gray-800 mb-2 break-words">{currentStepData.title}</h2>
                    {currentStepData.subtitle && <p className="text-sm sm:text-base text-gray-600 mb-6 md:mb-8 break-words">{currentStepData.subtitle}</p>}
                    <div className={`grid ${currentStepData.id === 'addons' ? 'grid-cols-1 sm:grid-cols-2' : currentStepData.options.length === 4 ? 'grid-cols-1 sm:grid-cols-2' : 'grid-cols-1'} gap-3 sm:gap-4 mb-6 md:mb-8`}>
                      {currentStepData.options.map((option) => {
                        ...
                        const isSelected = Array.isArray(selectedOptions) ? selectedOptions.includes(option.id) : selectedOptions === option.id;
                        ...
                        return (
                          <button
                            ...
                            className={`rounded-xl border-2 transition-all text-left w-full ${isAddonCard ? 'p-4 sm:p-5 shadow-sm hover:shadow-md' : 'p-3 sm:p-4'} ${isSelected ? 'border-[#cb530a] bg-[#fef3ed]' : 'border-gray-300 hover:border-[#cb530a]/50'}`}
                          >
                            <div className="flex items-start justify-between gap-2">
                              <div className="flex flex-col flex-1 min-w-0">
                                <div className="flex items-center flex-wrap gap-2">
                                  {Icon && <Icon className="w-5 h-5 sm:w-6 sm:h-6 text-[#cb530a] flex-shrink-0" />}
                                  <span className="font-semibold text-sm sm:text-base text-gray-800 break-words">{option.label}</span>
                                  {isAddonCard && isOptional && <span className="inline-flex items-center px-2 py-0.5 rounded text-xs font-medium bg-amber-100 text-amber-800 border border-amber-200">Optional</span>}
                                </div>
                                {isAddonCard && detail && <p className="mt-1.5 text-sm text-gray-600 break-words">{detail}</p>}
                              </div>
                              {isSelected && <div className="w-5 h-5 sm:w-6 sm:h-6 bg-[#cb530a] rounded-full flex items-center justify-center flex-shrink-0 mt-0.5"><Check className="w-3 h-3 sm:w-4 sm:h-4 text-white" /></div>}
                            </div>
                          </button>
                        );
                      })}
                    ...
                    <div className="flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-3 sm:gap-4">
                      <button type="button" onClick={handleBack} disabled={currentStep === 0} className="flex items-center justify-center px-4 sm:px-6 py-2.5 sm:py-3 text-sm sm:text-base text-gray-700 hover:text-[#cb530a] transition-colors disabled:opacity-50 disabled:cursor-not-allowed">
                        <ArrowLeft className="w-4 h-4 sm:w-5 sm:h-5 mr-2" /> Zurück
                      </button>
                      <button
                        type="button"
                        onClick={handleNext}
                        disabled={isAddonStep ? false : currentStepData.type === 'multiple' ? (selectedOptions as string[]).length === 0 : !selectedOptions}
      ...
      <Toast show={showToast} message={toastMessage} type={toastType} onClose={() => setShowToast(false)} duration={5000} />
    </div>
  );
}
