import { NextRequest, NextResponse } from 'next/server';
import { getAdminSession } from '@/lib/admin-auth';
import { getAuthUser, canAccessDigitalProducts } from '@/lib/auth';
import { supabaseAdmin } from '@/lib/supabase-admin';

async function requireDigitalProductsAccess(request: NextRequest) {
  const session = getAdminSession(request);
  if (!session) return { error: NextResponse.json({ error: 'Unauthorized' }, { status: 401 }) };
  const user = await getAuthUser(session.uid, session.tid);
  if (!user) return { error: NextResponse.json({ error: 'Benutzer nicht gefunden' }, { status: 401 }) };
  if (!canAccessDigitalProducts(user)) {
    return { error: NextResponse.json({ error: 'Kein Zugriff auf Digitale Produkte' }, { status: 403 }) };
  }
  return { session, user };
}

/** Prüft ob Datei zu einem Produkt des Tenants gehört */
async function fileBelongsToTenant(fileId: string, tenantId: string): Promise<boolean> {
  if (!supabaseAdmin) return false;
  const { data: file } = await supabaseAdmin
    .from('dp_product_files')
    .select('product_id')
    .eq('id', fileId)
    .maybeSingle();
  if (!file) return false;
  const { data: product } = await supabaseAdmin
    .from('dp_products')
    .select('id')
    .eq('id', file.product_id)
    .eq('tenant_id', tenantId)
    .maybeSingle();
  return !!product;
}

/** PATCH – Datei aktualisieren */
export async function PATCH(
  request: NextRequest,
  { params }: { params: Promise<{ fileId: string }> }
) {
  try {
    const auth = await requireDigitalProductsAccess(request);
    if (auth.error) return auth.error;
    const { session } = auth;
    const { fileId } = await params;

    const ok = await fileBelongsToTenant(fileId, session.tid);
    if (!ok) return NextResponse.json({ error: 'Datei nicht gefunden' }, { status: 404 });

    if (!supabaseAdmin) {
      return NextResponse.json({ error: 'Supabase nicht konfiguriert' }, { status: 500 });
    }

    const body = await request.json();
    const updates: Record<string, unknown> = { updated_at: new Date().toISOString() };
    const optional = ['title', 'file_type', 'file_url', 'sort_order'] as const;
    for (const key of optional) {
      if (body[key] !== undefined) {
        if (key === 'sort_order') updates[key] = Number(body[key]);
        else updates[key] = body[key];
      }
    }
    if (body.file_type && !['file', 'video_url', 'lesson'].includes(body.file_type)) {
      return NextResponse.json(
        { error: 'file_type muss file, video_url oder lesson sein' },
        { status: 400 }
      );
    }

    const { data, error } = await supabaseAdmin
      .from('dp_product_files')
      .update(updates)
      .eq('id', fileId)
      .select('*')
      .single();

    if (error) return NextResponse.json({ error: error.message }, { status: 500 });
    return NextResponse.json({ data }, { status: 200 });
  } catch (e) {
    console.error(e);
    return NextResponse.json({ error: 'Serverfehler' }, { status: 500 });
  }
}

/** DELETE – Datei löschen */
export async function DELETE(
  request: NextRequest,
  { params }: { params: Promise<{ fileId: string }> }
) {
  try {
    const auth = await requireDigitalProductsAccess(request);
    if (auth.error) return auth.error;
    const { session } = auth;
    const { fileId } = await params;

    const ok = await fileBelongsToTenant(fileId, session.tid);
    if (!ok) return NextResponse.json({ error: 'Datei nicht gefunden' }, { status: 404 });

    if (!supabaseAdmin) {
      return NextResponse.json({ error: 'Supabase nicht konfiguriert' }, { status: 500 });
    }

    const { error } = await supabaseAdmin.from('dp_product_files').delete().eq('id', fileId);

    if (error) return NextResponse.json({ error: error.message }, { status: 500 });
    return NextResponse.json({ success: true }, { status: 200 });
  } catch (e) {
    console.error(e);
    return NextResponse.json({ error: 'Serverfehler' }, { status: 500 });
  }
}
